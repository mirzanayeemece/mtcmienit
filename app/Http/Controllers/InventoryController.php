...
//------- METHODS FOR INVENTORY-SUPPLIER --------//
    ...
    public function inventory_supplier(){
        $inventory_supplier_info=DB::table('inventory_suppliers')
                           ->orderBy('id', 'desc')
                           ->get();
    ...
    public function save_inventory_supplier(Request $request)
    {
        $this->validate($request, [
          'name'  => ['required', 'string', 'max:100','unique:inventory_suppliers'],
        ...
        DB::table('inventory_suppliers')->insert($data);
    ...
    {
        $inventory_supplier=DB::table('inventory_suppliers')
                           ->where('id',$id)
                           ->first();
...
//------- METHODS FOR INVENTORY-RECEIVER --------//
    ...
    public function inventory_receiver(){
        $inventory_receiver_info=DB::table('inventory_receivers')
                           ->orderBy('id', 'desc')
                           ->get();
    ...
    {
        $this->validate($request, [
          'employee_id'  => ['required', 'integer', 'unique:inventory_receivers']
        ]);
        $data = array();
        $data['employee_id'] = $request->employee_id;
        $data['created_at'] = now();
        
        DB::table('inventory_receivers')->insert($data);
...
//------- METHODS FOR INVENTORY-CATEGORY --------//
    ...
    {
        $inventory_category=DB::table('inventory_categories')
                           ->where('id',$id)
                           ->first();
...
//------- METHODS FOR INVENTORY --------//
    ...
    {
        $this->validate($request, [
          'name'  => ['required', 'string', 'max:100','unique:inventories'],
    ...
    {
        $inventory_item=DB::table('inventories')
                           ->where('id',$id)
                           ->first();
        $inventory_category_info=DB::table('inventory_categories')
                            ->orderBy('id', 'desc')
